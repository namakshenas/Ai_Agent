        ...
        <article>
                ...
                <ul style={{minHeight : '118px'}}>
                    {getFilteredConversations().map((conversation, id) => 
                        ...
                        <li title={conversation.lastModelUsed || "no model assigned yet"} className="active" key={"conversation" + activePage*3+id} role="button">
                            <span>{conversation.history[0]?.question.substring(0, 45) || conversation.name}</span>
                            <button title="delete conversation" className='conversationTrashBtn' onClick={() => handleDeleteConversation(activePage*3+id)}>
