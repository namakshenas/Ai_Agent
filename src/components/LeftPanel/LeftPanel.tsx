...
export default function LeftPanel({activeConversation, setActiveConversation} : IProps){
    ...
    function nBlankConversationsNeededToFillListBlank() : number{
            ...
            <article>
                ...
                <ul style={{minHeight : '118px'}}>
                    {conversationsListState.slice(conversationsListPage*3, conversationsListPage*3+3).map((conversation, id) => (
                        ...
                        <li onClick={() => handleSetActiveConversation(conversationsListPage*3+id)} key={"conversation" + conversationsListPage*3+id} role="button">{conversation.history[0]?.question.substring(0, 45) || conversation.name}</li> : 
                    ...
                    {
                        nBlankConversationsNeededToFillListBlank() > 0 && Array(nBlankConversationsNeededToFillListBlank()).fill("").map((el,id) => (<li className='blankConversationItem' key={"blank"+id}></li>))
                    }
                </ul>
